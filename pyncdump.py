...
if len(ncdata["time"]) > 1:
    ...
    for var in vars_dic.keys():
        ...
        try:
            print(
                "\tVariable: {1}\n\t\t Epic Key: {0:_<10} :\t min={2:>15.3f} \t max={3:>15.3f} \t mean={4:>15.3f} \t median={5:>15.3f}"
            ).format(
                var,
                v_atts.long_name,
    ...
    if "time2" in vars_dic.keys():
        ...
        print("\t DeltaT based on first two points: {0} seconds").format(
            (ncdata["datetime"][1] - ncdata["datetime"][0]).seconds
        )
        print("\t DeltaT based on last two points: {0} seconds").format(
    ...
    if "time2" in vars_dic.keys():
        ...
        try:
            print("\t Depth Interval: {0} dBar").format(
