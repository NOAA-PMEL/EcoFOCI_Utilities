...
parser = argparse.ArgumentParser(
    description="Convert excel data into epic flavored netcdf files with CF times"
)
parser.add_argument(
    "ExcelDataPath",
...
parser.add_argument(
    "--latlon", nargs=2, type=float, help="latitude, longitude of mooring file"
)
parser.add_argument(
    "-fill_na", "--fill_na", action="store_true", help="fill nan with 1e35"
)

args = parser.parse_args()

wb_temp = pd.read_excel(
    args.ExcelDataPath, sheet_name="Temperature", na_values=[1e35, "1E+35", " 1E+35"]
)
wb_temp.rename(columns=lambda x: x.strip(), inplace=True)
if args.fill_na:
    wb_temp.fillna(1e35, inplace=True)

wb_pres = pd.read_excel(
    args.ExcelDataPath, sheet_name="Pressure", na_values=[1e35, "1E+35", " 1E+35"]
)
# wb_pres.rename(columns=lambda x: x.strip(), inplace=True)
if args.fill_na:
    wb_pres.fillna(1e35, inplace=True)

wb_sal = pd.read_excel(
    args.ExcelDataPath, sheet_name="Salinity", na_values=[1e35, "1E+35", " 1E+35"]
)
# wb_sal.rename(columns=lambda x: x.strip(), inplace=True)
if args.fill_na:
    wb_sal.fillna(1e35, inplace=True)

wb_date = pd.read_excel(
    args.ExcelDataPath,
    sheet_name="DateTime",
...
wb_coords = pd.read_excel(
    args.ExcelDataPath, sheet_name="Coords", na_values=[1e35, "1E+35", " 1E+35"]
)
...
ncinstance.dimension_init(
    time_len=len(wb_date.Date_Time.values),
    depth_len=len(wb_coords.Depth.to_dict().values()),
)
ncinstance.variable_init(EPIC_VARS_dict)
ncinstance.add_coord_data(
    depth=wb_coords.Depth.values,
    latitude=lat,
    longitude=lon,
    time=get_UDUNITS(datetime),
)
